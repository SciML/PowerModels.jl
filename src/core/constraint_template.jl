...
function constraint_gen_power_on_off(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_gen_power_on_off(
        pm, nw, i, gen["pmin"], gen["pmax"], gen["qmin"], gen["qmax"])
end
...
function constraint_network_power_balance(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    comp_gs = Dict(shunt["index"] => (shunt["shunt_bus"], shunt["gs"])
    for shunt in comp_shunts)
    comp_bs = Dict(shunt["index"] => (shunt["shunt_bus"], shunt["bs"])
    for shunt in comp_shunts)

    comp_branch_g = Dict(branch["index"] => (branch["f_bus"], branch["t_bus"],
                             branch["br_r"], branch["br_x"], branch["tap"],
                             branch["g_fr"], branch["g_to"]) for branch in comp_branches)
    comp_branch_b = Dict(branch["index"] => (branch["f_bus"], branch["t_bus"],
                             branch["br_r"], branch["br_x"], branch["tap"],
                             branch["b_fr"], branch["b_to"]) for branch in comp_branches)

    constraint_network_power_balance(pm, nw, i, comp_gen_ids, comp_pd, comp_qd,
        comp_gs, comp_bs, comp_branch_g, comp_branch_b)
end
...
function constraint_power_balance(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_power_balance(pm, nw, i, bus_arcs, bus_arcs_dc, bus_arcs_sw,
        bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
end
...
function constraint_power_balance_ls(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_power_balance_ls(pm, nw, i, bus_arcs, bus_arcs_dc, bus_arcs_sw,
        bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
end
...
function constraint_ne_power_balance(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_ne_power_balance(pm, nw, i, bus_arcs, bus_arcs_dc, bus_arcs_sw, bus_arcs_ne,
        bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
end
...
function constraint_current_balance(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_current_balance(
        pm, nw, i, bus_arcs, bus_arcs_dc, bus_gens, bus_pd, bus_qd, bus_gs, bus_bs)
end
...
function constraint_ohms_yt_from(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_ohms_yt_from(
        pm, nw, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
end
...
function constraint_ohms_yt_from_on_off(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_ohms_yt_from_on_off(pm, nw, i, f_bus, t_bus, f_idx, t_idx, g, b,
        g_fr, b_fr, tr, ti, tm, vad_min, vad_max)
end
...
function constraint_ohms_yt_to_on_off(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_ohms_yt_to_on_off(pm, nw, i, f_bus, t_bus, f_idx, t_idx, g, b,
        g_to, b_to, tr, ti, tm, vad_min, vad_max)
end
...
function constraint_ne_ohms_yt_from(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_ne_ohms_yt_from(pm, nw, i, f_bus, t_bus, f_idx, t_idx, g, b,
        g_fr, b_fr, tr, ti, tm, vad_min, vad_max)
end
...
function constraint_ne_ohms_yt_to(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_ne_ohms_yt_to(pm, nw, i, f_bus, t_bus, f_idx, t_idx, g, b,
        g_to, b_to, tr, ti, tm, vad_min, vad_max)
end
...
function constraint_power_magnitude_link(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_power_magnitude_link(
        pm, nw, f_bus, t_bus, arc_from, g, b, g_fr, b_fr, g_to, b_to, tr, ti, tm)
end
...
function constraint_power_magnitude_link_on_off(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_power_magnitude_link_on_off(
        pm, nw, i, arc_from, g, b, g_fr, b_fr, g_to, b_to, tr, ti, tm)
end
...
function constraint_thermal_limit_from_on_off(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    if !haskey(branch, "rate_a")
        Memento.error(_LOGGER,
            "constraint_thermal_limit_from_on_off requires a rate_a value on all branches, calc_thermal_limits! can be used to generate reasonable values")
    end
...
function constraint_thermal_limit_to_on_off(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    if !haskey(branch, "rate_a")
        Memento.error(_LOGGER,
            "constraint_thermal_limit_to_on_off requires a rate_a value on all branches, calc_thermal_limits! can be used to generate reasonable values")
    end
...
function constraint_ne_thermal_limit_from(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    if !haskey(branch, "rate_a")
        Memento.error(_LOGGER,
            "constraint_thermal_limit_from_ne requires a rate_a value on all branches, calc_thermal_limits! can be used to generate reasonable values")
    end
...
function constraint_ne_thermal_limit_to(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    if !haskey(branch, "rate_a")
        Memento.error(_LOGGER,
            "constraint_thermal_limit_to_ne requires a rate_a value on all branches, calc_thermal_limits! can be used to generate reasonable values")
    end
...
function constraint_voltage_angle_difference(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    if buspair["branch"] == i
        constraint_voltage_angle_difference(
            pm, nw, f_idx, buspair["angmin"], buspair["angmax"])
    end
end
...
function constraint_voltage_angle_difference_on_off(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_voltage_angle_difference_on_off(
        pm, nw, f_idx, branch["angmin"], branch["angmax"], vad_min, vad_max)
end
...
function constraint_power_losses_lb(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_power_losses_lb(
        pm, nw, f_bus, t_bus, f_idx, t_idx, g_fr, b_fr, g_to, b_to, tr)
end
...
function constraint_power_losses(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_power_losses(pm::AbstractPowerModel, nw, i, f_bus, t_bus, f_idx,
        t_idx, r, x, g_sh_fr, g_sh_to, b_sh_fr, b_sh_to, tm)
end
...
function constraint_voltage_magnitude_difference(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_voltage_magnitude_difference(
        pm, nw, i, f_bus, t_bus, f_idx, t_idx, r, x, g_sh_fr, b_sh_fr, tm)
end
...
function constraint_switch_on_off(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_switch_voltage_on_off(
        pm, nw, i, switch["f_bus"], switch["t_bus"], vad_min, vad_max)
end
...
function constraint_storage_current_limit(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_storage_current_limit(
        pm, nw, i, storage["storage_bus"], storage["current_rating"])
end
...
function constraint_storage_losses(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_storage_losses(pm, nw, i, storage["storage_bus"], storage["r"],
        storage["x"], storage["p_loss"], storage["q_loss"])
end
...
function constraint_storage_state(pm::AbstractPowerModel, i::Int; nw::Int = nw_id_default)
    ...
    constraint_storage_state_initial(
        pm, nw, i, storage["energy"], storage["charge_efficiency"],
        storage["discharge_efficiency"], time_elapsed)
end
...
function constraint_storage_state(pm::AbstractPowerModel, i::Int, nw_1::Int, nw_2::Int)
    ...
    if haskey(ref(pm, nw_1, :storage), i)
        constraint_storage_state(pm, nw_1, nw_2, i, storage["charge_efficiency"],
            storage["discharge_efficiency"], time_elapsed)
    else
        ...
        Memento.warn(_LOGGER,
            "storage component $(i) was not found in network $(nw_1) while building constraint_storage_state between networks $(nw_1) and $(nw_2). Using the energy value from the storage component in network $(nw_2) instead")
        constraint_storage_state_initial(
            pm, nw_2, i, storage["energy"], storage["charge_efficiency"],
            storage["discharge_efficiency"], time_elapsed)
    end
end
