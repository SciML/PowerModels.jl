...
"""
function sol_component_value_buspair(pm::AbstractPowerModel, n::Int, comp_name::Symbol,
        field_name::Symbol, variable_ids, variables)
...
"""
function variable_bus_voltage_angle(pm::AbstractPowerModel; nw::Int = nw_id_default,
...
"""
function variable_bus_voltage_magnitude(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
function variable_bus_voltage_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
real part of the voltage variable `i` in `bus`es
"""
function variable_bus_voltage_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
function variable_branch_voltage_magnitude_fr_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
    buses = ref(pm, nw, :bus)
...
"""
function variable_bus_voltage_magnitude_sqr(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
function variable_branch_voltage_magnitude_fr_sqr_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
    buses = ref(pm, nw, :bus)
    branches = ref(pm, nw, :branch)
...
"""
function variable_branch_voltage_magnitude_to_sqr_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
    buses = ref(pm, nw, :bus)
    branches = ref(pm, nw, :branch)
...
"""
"""
function variable_buspair_cosine(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report &&
        sol_component_value_buspair(pm, nw, :buspairs, :cs, ids(pm, nw, :buspairs), cs)
end

"""
"""
function variable_buspair_sine(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report &&
        sol_component_value_buspair(pm, nw, :buspairs, :si, ids(pm, nw, :buspairs), si)
end

"""
"""
function variable_buspair_voltage_product(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    if bounded
        wr_min, wr_max, wi_min,
        wi_max = ref_calc_voltage_product_bounds(ref(pm, nw, :buspairs))
...
"""
"""
function variable_branch_voltage_product_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
...
"""
function variable_gen_power_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
function variable_gen_power_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
function variable_gen_current_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    if bounded
        ...
        for (i, g) in ref(pm, nw, :gen)
            ...
            @assert vmin > 0
            s = sqrt(max(abs(g["pmax"]), abs(g["pmin"]))^2 +
                     max(abs(g["qmax"]), abs(g["qmin"]))^2)
            ub = s/vmin
...
"""
function variable_gen_current_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
function variable_gen_indicator(pm::AbstractPowerModel; nw::Int = nw_id_default,
        relax::Bool = false, report::Bool = true)
    if !relax
...
function variable_gen_power_real_on_off(pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
    pg = var(pm, nw)[:pg] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :gen)], base_name="$(nw)_pg",
...
function variable_gen_power_imaginary_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
    qg = var(pm, nw)[:qg] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :gen)], base_name="$(nw)_qg",
...
"""
function variable_branch_power_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    if bounded
        flow_lb,
        flow_ub = ref_calc_branch_flow_bounds(ref(pm, nw, :branch), ref(pm, nw, :bus))
    ...
    report && sol_component_value_edge(
        pm, nw, :branch, :pf, :pt, ref(pm, nw, :arcs_from), ref(pm, nw, :arcs_to), p)
end

"""
variable: `q[l,i,j]` for `(l,i,j)` in `arcs`
"""
function variable_branch_power_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report && sol_component_value_edge(
        pm, nw, :branch, :qf, :qt, ref(pm, nw, :arcs_from), ref(pm, nw, :arcs_to), q)
end
...
"""
function variable_branch_transform_magnitude(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
function variable_branch_transform_angle(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
function variable_branch_current_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report && sol_component_value_edge(pm, nw, :branch, :cr_fr, :cr_to,
        ref(pm, nw, :arcs_from), ref(pm, nw, :arcs_to), cr)
end

"""
variable: `ci[l,i,j] ` for `(l,i,j)` in `arcs`
"""
function variable_branch_current_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report && sol_component_value_edge(pm, nw, :branch, :ci_fr, :ci_to,
        ref(pm, nw, :arcs_from), ref(pm, nw, :arcs_to), ci)
end

"""
variable: `csr[l,i,j]` for `(l,i,j)` in `arcs_from`
"""
function variable_branch_series_current_real(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
function variable_branch_series_current_imaginary(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
function variable_dcline_power_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report &&
        sol_component_value_edge(pm, nw, :dcline, :pf, :pt, ref(pm, nw, :arcs_from_dc),
            ref(pm, nw, :arcs_to_dc), p_dc)
end

"""
variable: `q_dc[l,i,j]` for `(l,i,j)` in `arcs_dc`
"""
function variable_dcline_power_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report &&
        sol_component_value_edge(pm, nw, :dcline, :qf, :qt, ref(pm, nw, :arcs_from_dc),
            ref(pm, nw, :arcs_to_dc), q_dc)
end

"""
variable: `crdc[j]` for `j` in `dcline`
"""
function variable_dcline_current_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    if bounded
        ...
        for (l, i, j) in ref(pm, nw, :arcs_from_dc)
            ...
            @assert vmin_fr > 0
            @assert vmin_to > 0
            s_fr = sqrt(max(abs(dcline[l]["pmaxf"]), abs(dcline[l]["pminf"]))^2 +
                        max(abs(dcline[l]["qmaxf"]), abs(dcline[l]["qminf"]))^2)
            s_to = sqrt(max(abs(dcline[l]["pmaxt"]), abs(dcline[l]["pmint"]))^2 +
                        max(abs(dcline[l]["qmaxt"]), abs(dcline[l]["qmint"]))^2)
    ...
    report && sol_component_value_edge(pm, nw, :dcline, :crdc_fr, :crdc_to,
        ref(pm, nw, :arcs_from_dc), ref(pm, nw, :arcs_to_dc), crdc)
end

"""
variable:  `cidc[j]` for `j` in `dcline`
"""
function variable_dcline_current_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report && sol_component_value_edge(pm, nw, :dcline, :cidc_fr, :cidc_to,
        ref(pm, nw, :arcs_from_dc), ref(pm, nw, :arcs_to_dc), cidc)
end

"""
"""
function variable_switch_indicator(pm::AbstractPowerModel; nw::Int = nw_id_default,
        relax::Bool = false, report::Bool = true)
    if !relax
...
"""
function variable_switch_power_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    if bounded
        flow_lb,
        flow_ub = ref_calc_switch_flow_bounds(ref(pm, nw, :switch), ref(pm, nw, :bus))
    ...
    psw_expr = Dict{Any, Any}((l, i, j) => psw[(l, i, j)]
    for (l, i, j) in ref(pm, nw, :arcs_from_sw))
    psw_expr = merge(
        psw_expr, Dict((l, j, i) => -1.0*psw[(l, i, j)]
        for (l, i, j) in ref(pm, nw, :arcs_from_sw)))
    var(pm, nw)[:psw] = psw_expr

    report && sol_component_value_edge(
        pm, nw, :switch, :psw_fr, :psw_to, ref(pm, nw, :arcs_from_sw),
        ref(pm, nw, :arcs_to_sw), psw_expr)
end

"""
variable: `pws[l,i,j]` for `(l,i,j)` in `arcs_sw`
"""
function variable_switch_power_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    # this explicit type erasure is necessary
    qsw_expr = Dict{Any, Any}((l, i, j) => qsw[(l, i, j)]
    for (l, i, j) in ref(pm, nw, :arcs_from_sw))
    qsw_expr = merge(
        qsw_expr, Dict((l, j, i) => -1.0*qsw[(l, i, j)]
        for (l, i, j) in ref(pm, nw, :arcs_from_sw)))
    var(pm, nw)[:qsw] = qsw_expr

    report && sol_component_value_edge(
        pm, nw, :switch, :qsw_fr, :qsw_to, ref(pm, nw, :arcs_from_sw),
        ref(pm, nw, :arcs_to_sw), qsw_expr)
end
...
"""
"""
function variable_storage_power_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    if bounded
        inj_lb,
        inj_ub = ref_calc_storage_injection_bounds(ref(pm, nw, :storage), ref(pm, nw, :bus))
...
"""
"""
function variable_storage_power_imaginary(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
of the device.
"""
function variable_storage_power_control_imaginary(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report && sol_component_value(pm, nw, :storage, :qsc, ids(pm, nw, :storage), qsc)
end

"""
do nothing by default but some formulations require this
"""
function variable_storage_current(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
end

"""
"""
function variable_storage_energy(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
"""
function variable_storage_charge(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
"""
function variable_storage_discharge(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
"""
"""
function variable_storage_complementary_indicator(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        relax::Bool = false, report::Bool = true)
    if !relax
...
function variable_storage_power_real_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
    ps = var(pm, nw)[:ps] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :storage)], base_name="$(nw)_ps",
        start=comp_start_value(ref(pm, nw, :storage, i), "ps_start"))
...
function variable_storage_power_imaginary_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
    inj_lb,
    inj_ub = ref_calc_storage_injection_bounds(ref(pm, nw, :storage), ref(pm, nw, :bus))
...
of the device.
"""
function variable_storage_power_control_imaginary_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    qsc = var(pm, nw)[:qsc] = JuMP.@variable(pm.model,
        [i in ids(pm, nw, :storage)], base_name="$(nw)_qsc",
...
function variable_storage_indicator(pm::AbstractPowerModel; nw::Int = nw_id_default,
        relax::Bool = false, report::Bool = true)
    if !relax
    ...
    report &&
        sol_component_value(pm, nw, :storage, :status, ids(pm, nw, :storage), z_storage)
end
...
"""
function variable_ne_branch_power_real(pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    if bounded
        flow_lb,
        flow_ub = ref_calc_branch_flow_bounds(ref(pm, nw, :ne_branch), ref(pm, nw, :bus))
    ...
    report &&
        sol_component_value_edge(pm, nw, :ne_branch, :pf, :pt, ref(pm, nw, :ne_arcs_from),
            ref(pm, nw, :ne_arcs_to), p_ne)
end

"""
variable: `-ne_branch[l][\"rate_a\"] <= q_ne[l,i,j] <= ne_branch[l][\"rate_a\"]` for `(l,i,j)` in `ne_arcs`
"""
function variable_ne_branch_power_imaginary(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report &&
        sol_component_value_edge(pm, nw, :ne_branch, :qf, :qt, ref(pm, nw, :ne_arcs_from),
            ref(pm, nw, :ne_arcs_to), q_ne)
end

"""
variable: `0 <= z_branch[l] <= 1` for `l` in `branch`es
"""
function variable_branch_indicator(pm::AbstractPowerModel; nw::Int = nw_id_default,
        relax::Bool = false, report::Bool = true)
    if !relax
    ...
    report &&
        sol_component_value(pm, nw, :branch, :br_status, ids(pm, nw, :branch), z_branch)
end

"""
variable: `0 <= branch_ne[l] <= 1` for `l` in `branch`es
"""
function variable_ne_branch_indicator(pm::AbstractPowerModel; nw::Int = nw_id_default,
        relax::Bool = false, report::Bool = true)
    if !relax
    ...
    report && sol_component_value(
        pm, nw, :ne_branch, :built, ids(pm, nw, :ne_branch), z_branch_ne)
end

"""
"""
function variable_load_power_factor(pm::AbstractPowerModel; nw::Int = nw_id_default,
        relax::Bool = false, report::Bool = true)
    if !relax
    ...
    if report
        ...
        sol_pd = Dict(i => z_demand[i]*ref(pm, nw, :load, i)["pd"]
        for i in ids(pm, nw, :load))
        sol_component_value(pm, nw, :load, :pd, ids(pm, nw, :load), sol_pd)
        sol_qd = Dict(i => z_demand[i]*ref(pm, nw, :load, i)["qd"]
        for i in ids(pm, nw, :load))
...
"""
"""
function variable_shunt_admittance_factor(pm::AbstractPowerModel; nw::Int = nw_id_default,
        relax::Bool = false, report::Bool = true)
    if !relax
    ...
    if report
        ...
        sol_gs = Dict(i => z_shunt[i]*ref(pm, nw, :shunt, i)["gs"]
        for i in ids(pm, nw, :shunt))
        sol_component_value(pm, nw, :shunt, :gs, ids(pm, nw, :shunt), sol_gs)
        sol_bs = Dict(i => z_shunt[i]*ref(pm, nw, :shunt, i)["bs"]
        for i in ids(pm, nw, :shunt))
