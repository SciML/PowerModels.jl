...
function Base.show(io::IO, x::AdmittanceMatrix{<:Number})
    print(io, "AdmittanceMatrix($(length(x.idx_to_bus)) buses, $(length(nonzeros(x.matrix))) entries)")
end
...
function calc_admittance_matrix(data::Dict{String, <:Any})
    ...
    buses = [x.second
             for x in data["bus"]
             if (x.second[pm_component_status["bus"]] !=
                 pm_component_status_inactive["bus"])]
    ...
    for (i, branch) in data["branch"]
        f_bus = branch["f_bus"]
        t_bus = branch["t_bus"]
        if branch[pm_component_status["branch"]] !=
           pm_component_status_inactive["branch"] && haskey(bus_to_idx, f_bus) &&
           haskey(bus_to_idx, t_bus)
...
function Base.show(io::IO, x::AdmittanceMatrixInverse{<:Number})
    print(io, "AdmittanceMatrixInverse($(length(x.idx_to_bus)) buses, $(length(x.matrix)) entries)")
end
...
"""
function injection_factors_va(am_inv::AdmittanceMatrixInverse{T}, bus_id::Int)::Dict{
        Int, T} where {T}
...
"""
function injection_factors_va(am::AdmittanceMatrix{T}, ref_bus::Int, bus_id::Int)::Dict{
        Int, T} where {T}
    ...
    injection_factors = Dict(am.idx_to_bus[idx2_to_idx1[i]] => v
    for (i, v) in enumerate(if_vect) if !isapprox(v, 0.0))
