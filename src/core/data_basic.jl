...
function make_basic_network(data::Dict{String, <:Any})
    ...
    for (i, gen) in data["gen"]
        if get(gen, "cost_model", 2) != 2
            Memento.error(_LOGGER,
                "make_basic_network only supports network data with polynomial cost functions, generator $(i) has a piecewise linear cost function")
        end
    end
    ...
    for (comp_key, status_key) in pm_component_status
        ...
        data[comp_key] = _filter_inactive_components(
            data[comp_key], status_key = status_key,
            status_inactive_value = status_inactive)
        if length(data[comp_key]) < comp_count
            Memento.info(_LOGGER,
                "removed $(comp_count - length(data[comp_key])) inactive $(comp_key) components")
        end
    end
...
removed.
"""
function _filter_inactive_components(
        comp_dict::Dict{String, <:Any}; status_key = "status", status_inactive_value = 0)
...
function calc_basic_branch_series_impedance(data::Dict{String, <:Any})
    if !get(data, "basic_network", false)
        Memento.warn(_LOGGER,
            "calc_basic_branch_series_impedance requires basic network data and given data may be incompatible. make_basic_network can be used to transform data into the appropriate form.")
    end
...
function calc_basic_admittance_matrix(data::Dict{String, <:Any})
    if !get(data, "basic_network", false)
        Memento.warn(_LOGGER,
            "calc_basic_admittance_matrix requires basic network data and given data may be incompatible. make_basic_network can be used to transform data into the appropriate form.")
    end
...
function calc_basic_susceptance_matrix(data::Dict{String, <:Any})
    if !get(data, "basic_network", false)
        Memento.warn(_LOGGER,
            "calc_basic_susceptance_matrix requires basic network data and given data may be incompatible. make_basic_network can be used to transform data into the appropriate form.")
    end
...
function calc_basic_branch_susceptance_matrix(data::Dict{String, <:Any})
    if !get(data, "basic_network", false)
        Memento.warn(_LOGGER,
            "calc_basic_branch_susceptance_matrix requires basic network data and given data may be incompatible. make_basic_network can be used to transform data into the appropriate form.")
    end
...
function calc_basic_ptdf_row(data::Dict{String, <:Any}, branch_index::Int)
    ...
    if branch_index < 1 || branch_index > length(data["branch"])
        Memento.error(_LOGGER,
            "branch index of $(branch_index) is out of bounds, valid values are $(1)-$(length(data["branch"]))")
    end
    ...
    for i in 1:num_bus
        ...
        for j in neighbors[i]
            ...
            if bus_type == 1
                if i == j
                    y_ii = Y[i, i]
                    J[i1, j1] = + vm[i] * sum(-real(Y[i, k]) * vm[k] * sin(va[i] - va[k]) +
                                    imag(Y[i, k]) * vm[k] * cos(va[i] - va[k])
                    for k in neighbors[i] if k != i)
                    J[i1, j2] = + 2*real(y_ii)*vm[i] +
                                sum(real(Y[i, k]) * vm[k] * cos(va[i] - va[k]) +
                                    imag(Y[i, k]) * vm[k] * sin(va[i] - va[k])
                    for k in neighbors[i] if k != i)
                    J[i2, j1] = vm[i] * sum(real(Y[i, k]) * vm[k] * cos(va[i] - va[k]) +
                                    imag(Y[i, k]) * vm[k] * sin(va[i] - va[k])
                    for k in neighbors[i] if k != i)
                    J[i2, j2] = - 2*imag(y_ii)*vm[i] +
                                sum(real(Y[i, k]) * vm[k] * sin(va[i] - va[k]) -
                                    imag(Y[i, k]) * vm[k] * cos(va[i] - va[k])
                    for k in neighbors[i] if k != i)
                else
                    y_ij = Y[i, j]
                    J[i1, j1] = vm[i] * vm[j] *
                                (real(y_ij) * sin(va[i] - va[j]) -
                                 imag(y_ij) * cos(va[i] - va[j]))
                    J[i1, j2] = vm[i] * (real(y_ij) * cos(va[i] - va[j]) +
                                 imag(y_ij) * sin(va[i] - va[j]))
                    J[i2, j1] = vm[i] * vm[j] *
                                (-real(y_ij) * cos(va[i] - va[j]) -
                                 imag(y_ij) * sin(va[i] - va[j]))
                    J[i2, j2] = vm[i] * (real(y_ij) * sin(va[i] - va[j]) -
                                 imag(y_ij) * cos(va[i] - va[j]))
                end
            elseif bus_type == 2
                if i == j
