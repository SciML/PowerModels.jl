...
"""
function apply_pm!(func!::Function, ref::Dict{Symbol, <:Any},
        data::Dict{String, <:Any}; apply_to_subnetworks::Bool = true)
...
function calc_buspair_parameters(buses, branches, conductor_ids, ismulticondcutor)
    ...
    branch_lookup = Dict(branch["index"] => branch
    for (i, branch) in branches
    if branch["br_status"] == 1 && haskey(bus_lookup, branch["f_bus"]) &&
       haskey(bus_lookup, branch["t_bus"]))

    buspair_indexes = Set((branch["f_bus"], branch["t_bus"])
...
function ref_calc_voltage_product_bounds(buspairs, conductor::Int = 1)
    ...
    for (bp, buspair) in buspairs_conductor
        ...
        if buspair["angmin"] < 0 && buspair["angmax"] > 0
            ...
            wr_min[bp] = buspair["vm_fr_min"]*buspair["vm_to_min"]*min(
                cos(buspair["angmin"]), cos(buspair["angmax"]))
            wi_max[bp] = buspair["vm_fr_max"]*buspair["vm_to_max"]*sin(buspair["angmax"])
            wi_min[bp] = buspair["vm_fr_max"]*buspair["vm_to_max"]*sin(buspair["angmin"])
        end
    end
