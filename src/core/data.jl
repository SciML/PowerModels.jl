...
const _pm_component_parameter_order = Dict(
    ...
    "gen_status" => 81.0, "br_status" => 82.0, "model" => 90.0,
    "ncost" => 91.0, "cost" => 92.0, "startup" => 93.0, "shutdown" => 94.0
)
...
function component_table(data::Dict{String, <:Any}, component::String, args...)
...
function update_data!(data::Dict{String, <:Any}, new_data::Dict{String, <:Any})
    if haskey(data, "conductors") && haskey(new_data, "conductors")
        ...
        if (haskey(data, "conductors") && !haskey(new_data, "conductors")) ||
           (!haskey(data, "conductors") && haskey(new_data, "conductors"))
            Memento.warn(_LOGGER,
                "running update_data with missing onductors fields, conductors may be incorrect")
        end
    end
...
function _calc_branch_flow_ac(data::Dict{String, <:Any})
    ...
    for (i, branch) in data["branch"]
        if branch["br_status"] != 0
            ...
            p_to = (g+g_to)*vm_to^2 + (-g*tr-b*ti)/tm^2*(vm_to*vm_fr*cos(va_to-va_fr)) +
                   (-b*tr+g*ti)/tm^2*(vm_to*vm_fr*sin(va_to-va_fr))
            q_to = -(b+b_to)*vm_to^2 - (-b*tr+g*ti)/tm^2*(vm_to*vm_fr*cos(va_to-va_fr)) +
...
function _calc_power_balance(data::Dict{String, <:Any})
    ...
    for (i, bus) in data["bus"]
        if bus["bus_type"] != 4
            bvals = bus_values[bus["index"]]
            p_delta = bvals["p"] + bvals["p_dc"] + bvals["psw"] - bvals["pg"] +
                      bvals["ps"] + bvals["pd"] + bvals["gs"]*(bvals["vm"]^2)
            q_delta = bvals["q"] + bvals["q_dc"] + bvals["qsw"] - bvals["qg"] +
                      bvals["qs"] + bvals["qd"] - bvals["bs"]*(bvals["vm"]^2)
        else
            p_delta = NaN
            q_delta = NaN
        end
...
function correct_voltage_angle_differences!(data::Dict{String, <:Any}, default_pad = 1.0472)
    ...
    for c in 1:get(pm_data, "conductors", 1)
        ...
        for (i, branch) in pm_data["branch"]
            ...
            if angmin <= -pi/2
                Memento.warn(_LOGGER,
                    "this code only supports angmin values in -90 deg. to 90 deg., tightening the value on branch $i$(cnd_str) from $(rad2deg(angmin)) to -$(default_pad_deg) deg.")
            ...
            if angmax >= pi/2
                Memento.warn(_LOGGER,
                    "this code only supports angmax values in -90 deg. to 90 deg., tightening the value on branch $i$(cnd_str) from $(rad2deg(angmax)) to $(default_pad_deg) deg.")
                if haskey(pm_data, "conductors")
            ...
            if angmin == 0.0 && angmax == 0.0
                Memento.warn(_LOGGER,
                    "angmin and angmax values are 0, widening these values on branch $i$(cnd_str) to +/- $(default_pad_deg) deg.")
                if haskey(pm_data, "conductors")
                    branch["angmin"][c] = -default_pad
                    branch["angmax"][c] = default_pad
                else
    ...
    for branch in branches
        ...
        for c in 1:get(pm_data, "conductors", 1)
            ...
            if branch["rate_a"][c] <= 0.0
                ...
                Memento.warn(_LOGGER,
                    "this code only supports positive rate_a values, changing the value on branch $(branch["index"])$(cnd_str) to $(round(mva_base*new_rate, digits=4))")
    ...
    for branch in branches
        ...
        for c in 1:get(pm_data, "conductors", 1)
            ...
            if branch["c_rating_a"][c] <= 0.0
                ...
                new_c_rating = y_mag*sqrt(fr_vmax^2 + to_vmax^2 -
                                          2*fr_vmax*to_vmax*cos(theta_max))
...
function _check_storage_parameters(data::Dict{String, <:Any})
    for (i, strg) in data["storage"]
        ...
        for c in 1:get(data, "conductors", 1)
            if strg["r"][c] < 0.0
                Memento.error(_LOGGER,
                    "storage unit $(strg["index"]) has a non-positive resistance $(strg["r"][c])")
            end
            if strg["x"][c] < 0.0
                Memento.error(_LOGGER,
                    "storage unit $(strg["index"]) has a non-positive reactance $(strg["x"][c])")
            end
            if haskey(strg, "thermal_rating") && strg["thermal_rating"][c] < 0.0
                Memento.error(_LOGGER,
                    "storage unit $(strg["index"]) has a non-positive thermal rating $(strg["thermal_rating"][c])")
            end
            if haskey(strg, "current_rating") && strg["current_rating"][c] < 0.0
                Memento.error(_LOGGER,
                    "storage unit $(strg["index"]) has a non-positive current rating $(strg["thermal_rating"][c])")
            end
        end
    ...
    for c in 1:get(pm_data, "conductors", 1)
        ...
        for (i, dcline) in pm_data["dcline"]
            if dcline["loss0"][c] < 0.0
                new_rate = 0.0
                Memento.warn(_LOGGER,
                    "this code only supports positive loss0 values, changing the value on dcline $(dcline["index"])$(cnd_str) from $(mva_base*dcline["loss0"][c]) to $(mva_base*new_rate)")
                if haskey(pm_data, "conductors")
            ...
            if dcline["loss0"][c] >=
               dcline["pmaxf"][c]*(1-dcline["loss1"][c]) + dcline["pmaxt"][c]
                new_rate = 0.0
                Memento.warn(_LOGGER,
                    "this code only supports loss0 values which are consistent with the line flow bounds, changing the value on dcline $(dcline["index"])$(cnd_str) from $(mva_base*dcline["loss0"][c]) to $(mva_base*new_rate)")
                if haskey(pm_data, "conductors")
                    dcline["loss0"][c] = new_rate
                else
                    dcline["loss0"] = new_rate
            ...
            if dcline["loss1"][c] < 0.0
                new_rate = 0.0
                Memento.warn(_LOGGER,
                    "this code only supports positive loss1 values, changing the value on dcline $(dcline["index"])$(cnd_str) from $(dcline["loss1"][c]) to $(new_rate)")
                if haskey(pm_data, "conductors")
            ...
            if dcline["loss1"][c] >= 1.0
                new_rate = 0.0
                Memento.warn(_LOGGER,
                    "this code only supports loss1 values < 1, changing the value on dcline $(dcline["index"])$(cnd_str) from $(dcline["loss1"][c]) to $(new_rate)")
                if haskey(pm_data, "conductors")
                    dcline["loss1"][c] = new_rate
                else
                    dcline["loss1"] = new_rate
    ...
    for c in 1:get(data, "conductors", 1)
        ...
        for (i, gen) in data["gen"]
            ...
            if gen["vg"][c] != bus["vm"][c]
                Memento.warn(_LOGGER,
                    "the $(cnd_str)voltage setpoint on generator $(i) does not match the value at bus $(bus_id)")
            end
        end
        ...
        if comp["model"] == 1
            ...
            for i in 3:2:length(comp["cost"])
                if comp["cost"][i - 2] > comp["cost"][i]
                    Memento.error(_LOGGER,
                        "non-increasing x values in pwl cost model on $(type_name) $(id)")
                end
            end
...
function _deactivate_isolated_components!(data::Dict{String, <:Any})
            ...
            if bus["bus_type"] != 4
                ...
                if length(incident_branch[i]) + length(incident_dcline[i]) +
                   length(incident_switch[i]) > 0
                    incident_branch_count = sum([0;
                                                 [branch["br_status"]
                                                  for branch in incident_branch[i]]])
                    incident_dcline_count = sum([0;
                                                 [dcline["br_status"]
                                                  for dcline in incident_dcline[i]]])
                    incident_switch_count = sum([0;
                                                 [switch["status"]
                                                  for switch in incident_switch[i]]])
    ...
    for cc in ccs
        ...
        if (active_load_count == 0 && active_shunt_count == 0 && active_strg_count == 0) ||
           active_gen_count == 0
            Memento.info(_LOGGER,
                "deactivating connected component $(cc) due to isolation without generation, load or storage")
            for i in cc
                buses[i]["bus_type"] = 4
            end
            revised = true
...
function correct_component_refrence_bus!(component_bus_ids, bus_lookup, component_gens)
    ...
    if length(refrence_buses) == 0
        ...
        component_gens_active = Dict(k => v
        for (k, v) in component_gens if v["gen_status"] != 0)
...
"""
function bus_storage_lookup(storage_data::Dict{String, <:Any}, bus_data::Dict{
        String, <:Any})
...
"""
function calc_connected_components(data::Dict{String, <:Any}; edges = [
        "branch", "dcline", "switch"])
    pm_data = get_pm_data(data)
...
function _resolve_swithces!(data::Dict{String, <:Any})
    ...
    for (i, switch) in data["switch"]
        if switch[switch_status_key] != switch_status_value && switch["state"] == 1
            ...
            merged_set = Set{Int}([
                bus_sets[switch["f_bus"]]..., bus_sets[switch["t_bus"]]...])
