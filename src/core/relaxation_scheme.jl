...
function cut_complex_product_and_angle_difference(m, wf, wt, wr, wi, angmin, angmax)
    ...
    JuMP.@constraint(m,
        sf*st*(cos(phi)*wr + sin(phi)*wi) - vt_ub*cos(d)*st*wf - vf_ub*cos(d)*sf*wt >=
        vf_ub*vt_ub*cos(d)*(vf_lb*vt_lb - vf_ub*vt_ub))
    JuMP.@constraint(m,
        sf*st*(cos(phi)*wr + sin(phi)*wi) - vt_lb*cos(d)*st*wf - vf_lb*cos(d)*sf*wt >=
        -vf_lb*vt_lb*cos(d)*(vf_lb*vt_lb - vf_ub*vt_ub))
end
...
function relaxation_sin_on_off(m, x, y, z, M_x)
    ...
    JuMP.@constraint(m,
        y - cos(max_ad/2)*(x) <=
        z*(sin(max_ad/2) - cos(max_ad/2)*max_ad/2) + (1-z)*(cos(max_ad/2)*M_x))
    JuMP.@constraint(m,
        -y + cos(max_ad/2)*(x) <=
        z*(sin(max_ad/2) - cos(max_ad/2)*max_ad/2) + (1-z)*(cos(max_ad/2)*M_x))
    ...
    JuMP.@constraint(m,
        cos(max_ad/2)*x <=
        z*(sin(max_ad/2) - cos(max_ad/2)*max_ad/2 + sin(max_ad)) +
        (1-z)*(cos(max_ad/2)*M_x))
    JuMP.@constraint(m,
        -cos(max_ad/2)*x <=
        z*(sin(max_ad/2) - cos(max_ad/2)*max_ad/2 + sin(max_ad)) +
        (1-z)*(cos(max_ad/2)*M_x))
...
function relaxation_cos_on_off(m, x, y, z, M_x)
    ...
    JuMP.@constraint(m,
        y <=
        z - (1-cos(max_ad))/(max_ad^2)*(x^2) + (1-z)*((1-cos(max_ad))/(max_ad^2)*(M_x^2)))
end
