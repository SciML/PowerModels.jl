...
"""
"""
function ismulticonductor(pm::AbstractPowerModel, nw::Int)
    haskey(pm.ref[:it][pm_it_sym][:nw][nw], :conductors)
end
function ismulticonductor(pm::AbstractPowerModel; nw::Int = nw_id_default)
    haskey(pm.ref[:it][pm_it_sym][:nw][nw], :conductors)
end

"""
"""
function conductor_ids(pm::AbstractPowerModel, nw::Int)
    pm.ref[:it][pm_it_sym][:nw][nw][:conductor_ids]
end
function conductor_ids(pm::AbstractPowerModel; nw::Int = nw_id_default)
    pm.ref[:it][pm_it_sym][:nw][nw][:conductor_ids]
end
...
function solve_model(data::Dict{String, <:Any}, model_type::Type, optimizer, build_method;
    ...
    pm = instantiate_model(
        data, model_type, build_method; ref_extensions = ref_extensions, kwargs...)
    Memento.debug(_LOGGER, "pm model build time: $(time() - start_time)")

    start_time = time()
    result = optimize_model!(
        pm, relax_integrality = relax_integrality, optimizer = optimizer,
...
function instantiate_model(data::Dict{String, <:Any}, model_type::Type, build_method; kwargs...)
    return _IM.instantiate_model(data, model_type, build_method, ref_add_core!,
        _pm_global_keys, pm_it_sym; kwargs...)
end
...
function build_ref(data::Dict{String, <:Any}; ref_extensions = [])
    return _IM.build_ref(
        data, ref_add_core!, _pm_global_keys, pm_it_name; ref_extensions = ref_extensions)
end
...
function ref_add_core!(ref::Dict{Symbol, Any})
    for (nw, nw_ref) in ref[:it][pm_it_sym][:nw]
        ...
        nw_ref[:bus] = Dict(x
        for x in nw_ref[:bus]
        if (x.second["bus_type"] != pm_component_status_inactive["bus"]))
        nw_ref[:load] = Dict(x
        for x in nw_ref[:load]
        if (x.second["status"] != pm_component_status_inactive["load"] &&
            x.second["load_bus"] in keys(nw_ref[:bus])))
        nw_ref[:shunt] = Dict(x
        for x in nw_ref[:shunt]
        if (x.second["status"] != pm_component_status_inactive["shunt"] &&
            x.second["shunt_bus"] in keys(nw_ref[:bus])))
        nw_ref[:gen] = Dict(x
        for x in nw_ref[:gen]
        if (x.second["gen_status"] != pm_component_status_inactive["gen"] &&
            x.second["gen_bus"] in keys(nw_ref[:bus])))
        nw_ref[:storage] = Dict(x
        for x in nw_ref[:storage]
        if (x.second["status"] != pm_component_status_inactive["storage"] &&
            x.second["storage_bus"] in keys(nw_ref[:bus])))
        nw_ref[:switch] = Dict(x
        for x in nw_ref[:switch]
        if (x.second["status"] != pm_component_status_inactive["switch"] &&
            x.second["f_bus"] in keys(nw_ref[:bus]) &&
            x.second["t_bus"] in keys(nw_ref[:bus])))
        nw_ref[:branch] = Dict(x
        for x in nw_ref[:branch]
        if (x.second["br_status"] != pm_component_status_inactive["branch"] &&
            x.second["f_bus"] in keys(nw_ref[:bus]) &&
            x.second["t_bus"] in keys(nw_ref[:bus])))
        nw_ref[:dcline] = Dict(x
        for x in nw_ref[:dcline]
        if (x.second["br_status"] != pm_component_status_inactive["dcline"] &&
            x.second["f_bus"] in keys(nw_ref[:bus]) &&
            x.second["t_bus"] in keys(nw_ref[:bus])))

        ### setup arcs from edges ###
        nw_ref[:arcs_from] = [(i, branch["f_bus"], branch["t_bus"])
                              for (i, branch) in nw_ref[:branch]]
        nw_ref[:arcs_to] = [(i, branch["t_bus"], branch["f_bus"])
                            for (i, branch) in nw_ref[:branch]]
        nw_ref[:arcs] = [nw_ref[:arcs_from]; nw_ref[:arcs_to]]

        nw_ref[:arcs_from_dc] = [(i, dcline["f_bus"], dcline["t_bus"])
                                 for (i, dcline) in nw_ref[:dcline]]
        nw_ref[:arcs_to_dc] = [(i, dcline["t_bus"], dcline["f_bus"])
                               for (i, dcline) in nw_ref[:dcline]]
        nw_ref[:arcs_dc] = [nw_ref[:arcs_from_dc]; nw_ref[:arcs_to_dc]]

        nw_ref[:arcs_from_sw] = [(i, switch["f_bus"], switch["t_bus"])
                                 for (i, switch) in nw_ref[:switch]]
        nw_ref[:arcs_to_sw] = [(i, switch["t_bus"], switch["f_bus"])
                               for (i, switch) in nw_ref[:switch]]
        ...
        if !haskey(nw_ref, :buspairs)
            nw_ref[:buspairs] = calc_buspair_parameters(nw_ref[:bus], nw_ref[:branch],
                nw_ref[:conductor_ids], haskey(nw_ref, :conductors))
        end
    end
end
...
function ids(pm::AbstractPowerModel, key::Symbol; nw::Int = nw_id_default)
...
function ref(pm::AbstractPowerModel, nw::Int, key::Symbol, idx)
    _IM.ref(pm, pm_it_sym, nw, key, idx)
end
function ref(pm::AbstractPowerModel, nw::Int, key::Symbol, idx, param::String)
    _IM.ref(pm, pm_it_sym, nw, key, idx, param)
end
function ref(pm::AbstractPowerModel, key::Symbol; nw::Int = nw_id_default)
    _IM.ref(pm, pm_it_sym, key; nw = nw)
end
function ref(pm::AbstractPowerModel, key::Symbol, idx; nw::Int = nw_id_default)
    _IM.ref(pm, pm_it_sym, key, idx; nw = nw)
end
function ref(
        pm::AbstractPowerModel, key::Symbol, idx, param::String; nw::Int = nw_id_default)
...
function var(pm::AbstractPowerModel, nw::Int, key::Symbol, idx)
    _IM.var(pm, pm_it_sym, nw, key, idx)
end
function var(pm::AbstractPowerModel, key::Symbol; nw::Int = nw_id_default)
    _IM.var(pm, pm_it_sym, key; nw = nw)
end
function var(pm::AbstractPowerModel, key::Symbol, idx; nw::Int = nw_id_default)
...
function con(pm::AbstractPowerModel, nw::Int, key::Symbol, idx)
    _IM.con(pm, pm_it_sym, nw, key, idx)
end
function con(pm::AbstractPowerModel, key::Symbol; nw::Int = nw_id_default)
    _IM.con(pm, pm_it_sym, key; nw = nw)
end
function con(pm::AbstractPowerModel, key::Symbol, idx; nw::Int = nw_id_default)
...
function sol(pm::AbstractPowerModel, nw::Int, key::Symbol, idx)
    _IM.sol(pm, pm_it_sym, nw, key, idx)
end
function sol(pm::AbstractPowerModel, key::Symbol; nw::Int = nw_id_default)
    _IM.sol(pm, pm_it_sym, key; nw = nw)
end
function sol(pm::AbstractPowerModel, key::Symbol, idx; nw::Int = nw_id_default)
