...
function check_cost_models(pm::AbstractPowerModel)
    ...
    if gen_model != dcline_model
        Memento.error(_LOGGER,
            "generator and dcline cost models are inconsistent, the generator model is $(gen_model) however dcline model $(dcline_model)")
    end
        ...
        for (i, gen) in nw_ref[:gen]
            if haskey(gen, "cost")
                if model == nothing
                    ...
                    if gen["model"] != model
                        Memento.error(_LOGGER,
                            "cost models are inconsistent, the typical model is $(model) however model $(gen["model"]) is given on generator $(i)")
                    end
                end
            else
            ...
            if haskey(dcline, "model")
                if model == nothing
                    ...
                    if dcline["model"] != model
                        Memento.error(_LOGGER,
                            "cost models are inconsistent, the typical model is $(model) however model $(dcline["model"]) is given on dcline $(i)")
                    end
                end
            else
...
function objective_min_fuel_and_flow_cost(pm::AbstractPowerModel; kwargs...)
    ...
    elseif model == 2
        ...
        Memento.error(_LOGGER,
            "Only cost models of types 1 and 2 are supported at this time, given cost model type of $(model)")
    end
end
    ...
    for (n, nw_ref) in nws(pm)
        ...
        for (i, dcline) in nw_ref[:dcline]
            ...
            elseif length(dcline["cost"]) == 3
                dcline_cost[(n, i)] = dcline["cost"][1]*p_dc^2 + dcline["cost"][2]*p_dc +
                                      dcline["cost"][3]
            else
                dcline_cost[(n, i)] = 0.0
            end
    ...
    for (n, nw_ref) in nws(pm)
        ...
        for (i, gen) in nw_ref[:gen]
            ...
            elseif length(gen["cost"]) == 3
                ...
                JuMP.@constraint(pm.model,
                    [0.5, pg_sqr, pg] in JuMP.RotatedSecondOrderCone())

                gen_cost[(n, i)] = gen["cost"][1]*pg_sqr + gen["cost"][2]*pg +
                                   gen["cost"][3]
            else
                gen_cost[(n, i)] = 0.0
            end
        ...
        for (i, dcline) in nw_ref[:dcline]
            ...
            elseif length(dcline["cost"]) == 3
                ...
                JuMP.@constraint(pm.model,
                    [0.5, p_dc_sqr, p_dc] in JuMP.RotatedSecondOrderCone())

                dcline_cost[(n, i)] = dcline["cost"][1]*p_dc_sqr + dcline["cost"][2]*p_dc +
                                      dcline["cost"][3]
            else
                dcline_cost[(n, i)] = 0.0
            end
        ...
        for (i, gen) in nw_ref[:gen]
            ...
            elseif length(cost_rev) == 2
                gen_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    cost_rev[1] + cost_rev[2]*pg)
            elseif length(cost_rev) == 3
                gen_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    cost_rev[1] + cost_rev[2]*pg + cost_rev[3]*pg^2)
            elseif length(cost_rev) >= 4
                cost_rev_nl = cost_rev[4:end]
                gen_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    cost_rev[1] + cost_rev[2]*pg + cost_rev[3]*pg^2 +
                    sum(v*pg^(d+2) for (d, v) in enumerate(cost_rev_nl)))
            else
        ...
        for (i, dcline) in nw_ref[:dcline]
            ...
            elseif length(cost_rev) == 2
                dcline_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    cost_rev[1] + cost_rev[2]*p_dc)
            elseif length(cost_rev) == 3
                dcline_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    cost_rev[1] + cost_rev[2]*p_dc + cost_rev[3]*p_dc^2)
            elseif length(cost_rev) >= 4
                cost_rev_nl = cost_rev[4:end]
                dcline_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    cost_rev[1] + cost_rev[2]*p_dc + cost_rev[3]*p_dc^2 +
                    sum(v*p_dc^(d+2) for (d, v) in enumerate(cost_rev_nl)))
            else
...
"""
function calc_pwl_points(
        ncost::Int, cost::Vector{<:Real}, pmin::Real, pmax::Real; tolerance = 1e-2)
    ...
    if isinf(pmin) || isinf(pmax)
        Memento.error(_LOGGER,
            "a bounded operating range is required for modeling pwl costs.  Given active power range in $(pmin) - $(pmax)")
    end
...
function objective_variable_dc_cost(pm::AbstractPowerModel, report::Bool = true)
    for (n, nw_ref) in nws(pm)
        ...
        report &&
            sol_component_value(pm, n, :dcline, :p_dc_cost, ids(pm, n, :dcline), p_dc_cost)
    end
end
