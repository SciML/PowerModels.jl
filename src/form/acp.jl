...
function constraint_power_balance(
        pm::AbstractACPModel, n::Int, i::Int, bus_arcs, bus_arcs_dc,
        bus_arcs_sw, bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
    vm = var(pm, n, :vm, i)
    ...
    # remove this.
    nl_form = length(bus_arcs) > 0 &&
              (typeof(p[iterate(bus_arcs)[1]]) <: JuMP.NonlinearExpression)
...
"""
"""
function constraint_power_balance_ls(
        pm::AbstractACPModel, n::Int, i::Int, bus_arcs, bus_arcs_dc,
        bus_arcs_sw, bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
    vm = var(pm, n, :vm, i)
    p = get(var(pm, n), :p, Dict());
    ...
    z_demand = get(var(pm, n), :z_demand, Dict());
    _check_var_keys(z_demand, keys(bus_pd), "power factor", "load")
    z_shunt = get(var(pm, n), :z_shunt, Dict());
    _check_var_keys(z_shunt, keys(bus_gs), "power factor", "shunt")
...
function constraint_ne_power_balance(
        pm::AbstractACPModel, n::Int, i::Int, bus_arcs, bus_arcs_dc, bus_arcs_sw,
        bus_arcs_ne, bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
    vm = var(pm, n, :vm, i)
    p = get(var(pm, n), :p, Dict());
    _check_var_keys(p, bus_arcs, "active power", "branch")
    q = get(var(pm, n), :q, Dict());
...
"""
"""
function expression_branch_power_ohms_yt_from(pm::AbstractACPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)

    var(pm, n, :p)[f_idx] = JuMP.@NLexpression(pm.model,
        (g+g_fr)/tm^2*vm_fr^2 + (-g*tr+b*ti)/tm^2*(vm_fr*vm_to*cos(va_fr-va_to)) +
        (-b*tr-g*ti)/tm^2*(vm_fr*vm_to*sin(va_fr-va_to)))
    var(pm, n, :q)[f_idx] = JuMP.@NLexpression(pm.model,
        -(b+b_fr)/tm^2*vm_fr^2 - (-b*tr-g*ti)/tm^2*(vm_fr*vm_to*cos(va_fr-va_to)) +
        (-g*tr+b*ti)/tm^2*(vm_fr*vm_to*sin(va_fr-va_to)))
end

"""
"""
function expression_branch_power_ohms_yt_to(pm::AbstractACPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)

    var(pm, n, :p)[t_idx] = JuMP.@NLexpression(pm.model,
        (g+g_to)*vm_to^2 + (-g*tr-b*ti)/tm^2*(vm_to*vm_fr*cos(va_to-va_fr)) +
        (-b*tr+g*ti)/tm^2*(vm_to*vm_fr*sin(va_to-va_fr)))
    var(pm, n, :q)[t_idx] = JuMP.@NLexpression(pm.model,
        -(b+b_to)*vm_to^2 - (-b*tr+g*ti)/tm^2*(vm_to*vm_fr*cos(va_to-va_fr)) +
...
```
"""
function constraint_ohms_yt_from(pm::AbstractACPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
    vm_fr = var(pm, n, :vm, f_bus)
...
```
"""
function constraint_ohms_yt_to(pm::AbstractACPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm)
    p_to = var(pm, n, :p, t_idx)
    q_to = var(pm, n, :q, t_idx)
    vm_fr = var(pm, n, :vm, f_bus)
...
```
"""
function constraint_ohms_y_from(
        pm::AbstractACPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tm, ta)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    ...
    JuMP.@NLconstraint(pm.model,
        p_fr ==
        (g+g_fr)*(vm_fr/tm)^2 - g*vm_fr/tm*vm_to*cos(va_fr-va_to-ta) +
        -b*vm_fr/tm*vm_to*sin(va_fr-va_to-ta))
    JuMP.@NLconstraint(pm.model,
        q_fr ==
        -(b+b_fr)*(vm_fr/tm)^2 + b*vm_fr/tm*vm_to*cos(va_fr-va_to-ta) +
        -g*vm_fr/tm*vm_to*sin(va_fr-va_to-ta))
end
...
```
"""
function constraint_ohms_y_to(
        pm::AbstractACPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tm, ta)
    p_to = var(pm, n, :p, t_idx)
    q_to = var(pm, n, :q, t_idx)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    ...
    JuMP.@NLconstraint(pm.model,
        p_to ==
        (g+g_to)*vm_to^2 - g*vm_to*vm_fr/tm*cos(va_to-va_fr+ta) +
        -b*vm_to*vm_fr/tm*sin(va_to-va_fr+ta))
    JuMP.@NLconstraint(pm.model,
        q_to ==
        -(b+b_to)*vm_to^2 + b*vm_to*vm_fr/tm*cos(va_to-va_fr+ta) +
        -g*vm_to*vm_fr/tm*sin(va_to-va_fr+ta))
end
...
function constraint_switch_voltage_on_off(
        pm::AbstractACPModel, n::Int, i, f_bus, t_bus, vad_min, vad_max)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
function constraint_ohms_yt_from_on_off(
        pm::AbstractACPModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_fr, b_fr, tr, ti, tm, vad_min, vad_max)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    ...
    JuMP.@NLconstraint(pm.model,
        p_fr ==
        z*((g+g_fr)/tm^2*vm_fr^2 + (-g*tr+b*ti)/tm^2*(vm_fr*vm_to*cos(va_fr-va_to)) +
           (-b*tr-g*ti)/tm^2*(vm_fr*vm_to*sin(va_fr-va_to))))
    JuMP.@NLconstraint(pm.model,
        q_fr ==
        z*(-(b+b_fr)/tm^2*vm_fr^2 - (-b*tr-g*ti)/tm^2*(vm_fr*vm_to*cos(va_fr-va_to)) +
           (-g*tr+b*ti)/tm^2*(vm_fr*vm_to*sin(va_fr-va_to))))
end
...
```
"""
function constraint_ohms_yt_to_on_off(pm::AbstractACPModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_to, b_to, tr, ti, tm, vad_min, vad_max)
    p_to = var(pm, n, :p, t_idx)
    q_to = var(pm, n, :q, t_idx)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    ...
    JuMP.@NLconstraint(pm.model,
        p_to ==
        z*((g+g_to)*vm_to^2 + (-g*tr-b*ti)/tm^2*(vm_to*vm_fr*cos(va_to-va_fr)) +
           (-b*tr+g*ti)/tm^2*(vm_to*vm_fr*sin(va_to-va_fr))))
    JuMP.@NLconstraint(pm.model,
        q_to ==
        z*(-(b+b_to)*vm_to^2 - (-b*tr+g*ti)/tm^2*(vm_to*vm_fr*cos(va_to-va_fr)) +
           (-g*tr-b*ti)/tm^2*(vm_to*vm_fr*sin(va_to-va_fr))))
end
...
```
"""
function constraint_ne_ohms_yt_from(pm::AbstractACPModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_fr, b_fr, tr, ti, tm, vad_min, vad_max)
...
```
"""
function constraint_ne_ohms_yt_to(pm::AbstractACPModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_to, b_to, tr, ti, tm, vad_min, vad_max)
...
```
"""
function constraint_ohms_y_oltc_pst_from(
        pm::AbstractACPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    ...
    JuMP.@NLconstraint(pm.model,
        p_fr ==
        (g+g_fr)/tm^2*vm_fr^2 + (-g)/tm*(vm_fr*vm_to*cos(va_fr-va_to-ta)) +
        (-b)/tm*(vm_fr*vm_to*sin(va_fr-va_to-ta)))
    JuMP.@NLconstraint(pm.model,
        q_fr ==
        -(b+b_fr)/tm^2*vm_fr^2 - (-b)/tm*(vm_fr*vm_to*cos(va_fr-va_to-ta)) +
        (-g)/tm*(vm_fr*vm_to*sin(va_fr-va_to-ta)))
end
...
```
"""
function constraint_ohms_y_oltc_pst_to(
        pm::AbstractACPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to)
    p_to = var(pm, n, :p, t_idx)
    q_to = var(pm, n, :q, t_idx)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    ...
    JuMP.@NLconstraint(pm.model,
        p_to ==
        (g+g_to)*vm_to^2 + -g/tm*(vm_to*vm_fr*cos(va_to-va_fr+ta)) +
        -b/tm*(vm_to*vm_fr*sin(va_to-va_fr+ta)))
    JuMP.@NLconstraint(pm.model,
        q_to ==
        -(b+b_to)*vm_to^2 - -b/tm*(vm_to*vm_fr*cos(va_to-va_fr+ta)) +
        -g/tm*(vm_to*vm_fr*sin(va_to-va_fr+ta)))
end

"""
"""
function constraint_ohms_y_pst_from(
        pm::AbstractACPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tm)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
...
function constraint_ohms_y_pst_to(
        pm::AbstractACPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tm)
    p_to = var(pm, n, :p, t_idx)
    q_to = var(pm, n, :q, t_idx)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
...
function constraint_voltage_angle_difference_on_off(
        pm::AbstractACPModel, n::Int, f_idx, angmin, angmax, vad_min, vad_max)
    i, f_bus, t_bus = f_idx
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
    z = var(pm, n, :z_branch, i)
...
```
"""
function constraint_power_losses_lb(pm::AbstractACPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g_fr, b_fr, g_to, b_to, tr)
    vm_fr = var(pm, n, :vm, f_bus)
    vm_to = var(pm, n, :vm, t_bus)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
...
function constraint_storage_losses(
        pm::AbstractACPModel, n::Int, i, bus, r, x, p_loss, q_loss; conductors = [1])
    vm = var(pm, n, :vm, bus)
    ps = var(pm, n, :ps, i)
    qs = var(pm, n, :qs, i)
    sc = var(pm, n, :sc, i)
