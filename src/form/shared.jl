...
"""
"""
function variable_shunt_admittance_factor(
        pm::AbstractWConvexModels; nw::Int = nw_id_default,
...
"""
function variable_storage_current(pm::AbstractWConvexModels; nw::Int = nw_id_default,
...
function constraint_voltage_angle_difference(
        pm::AbstractPolarModels, n::Int, f_idx, angmin, angmax)
    i, f_bus, t_bus = f_idx
...
function constraint_power_balance(
        pm::AbstractWModels, n::Int, i, bus_arcs, bus_arcs_dc, bus_arcs_sw,
...
function constraint_power_balance_ls(
        pm::AbstractWConvexModels, n::Int, i::Int, bus_arcs, bus_arcs_dc,
    ...
    wz_shunt = get(var(pm, n), :wz_shunt, Dict());
    _check_var_keys(wz_shunt, keys(bus_gs), "voltage square power factor", "shunt")
...
function constraint_ne_power_balance(
        pm::AbstractWModels, n::Int, i::Int, bus_arcs, bus_arcs_dc, bus_arcs_sw,
...
"""
"""
function expression_branch_power_ohms_yt_from(pm::AbstractWRModels, n::Int, f_bus, t_bus,
    ...
    var(pm, n, :q)[f_idx] = -(b+b_fr)/tm^2*w_fr - (-b*tr-g*ti)/tm^2*wr +
                            (-g*tr+b*ti)/tm^2*wi
end

"""
"""
function expression_branch_power_ohms_yt_to(pm::AbstractWRModels, n::Int, f_bus, t_bus,
...
"""
function constraint_ohms_yt_from(pm::AbstractWRModels, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
...
Creates Ohms constraints (yt post fix indicates that Y and T values are in rectangular form)
"""
function constraint_ohms_yt_to(pm::AbstractWRModels, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm)
...
"""
function constraint_voltage_angle_difference(
        pm::AbstractWModels, n::Int, f_idx, angmin, angmax)
    i, f_bus, t_bus = f_idx
...
function constraint_network_power_balance(
        pm::AbstractWModels, n::Int, i, comp_gen_ids, comp_pd,
    ...
    JuMP.@constraint(pm.model,
        ...
        sum(gs*w[i] for (i, gs) in values(comp_gs)) +
        sum(g_fr*w[i]/tm^2 + g_to*w[j]
        for (i, j, r, x, tm, g_fr, g_to) in values(comp_branch_g)))
    JuMP.@constraint(pm.model,
        sum(qg[g] for g in comp_gen_ids) >=
        sum(qd for (i, qd) in values(comp_qd)) -
        sum(bs*w[i] for (i, bs) in values(comp_bs)) - sum(b_fr*w[i]/tm^2 + b_to*w[j]
    for (i, j, r, x, tm, b_fr, b_to) in values(comp_branch_b)))
end
...
function constraint_switch_voltage_on_off(
        pm::AbstractWModels, n::Int, i, f_bus, t_bus, vad_min, vad_max)
    w_fr = var(pm, n, :w, f_bus)
    w_to = var(pm, n, :w, t_bus)
...
function constraint_storage_losses(
        pm::AbstractWConvexModels, n::Int, i, bus, r, x, p_loss, q_loss; conductors = [1])
    w = var(pm, n, :w, bus)
