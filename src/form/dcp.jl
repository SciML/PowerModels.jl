...
"""
"""
function variable_bus_voltage_magnitude(pm::AbstractDCPModel; nw::Int = nw_id_default,
...
```
"""
function constraint_ohms_yt_from(pm::AbstractDCPModel, n::Int, f_bus, t_bus,
...
"""
"""
function expression_branch_power_ohms_yt_from(pm::AbstractDCPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
"""
"""
function expression_branch_power_ohms_yt_to(pm::AbstractDCPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
function constraint_ne_ohms_yt_from(pm::AbstractDCPModel, n::Int, i, f_bus, t_bus, f_idx,
...
function constraint_ne_ohms_yt_from(pm::AbstractDCMPPModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_fr, b_fr, tr, ti, tm, vad_min, vad_max)
    p_fr = var(pm, n, :p_ne, f_idx)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
"""
"""
function expression_branch_power_ohms_yt_from(pm::AbstractDCMPPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
"""
"""
function expression_branch_power_ohms_yt_to(pm::AbstractDCMPPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
"""
"""
function constraint_ohms_yt_from(pm::AbstractDCMPPModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    p_fr = var(pm, n, :p, f_idx)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
function constraint_ohms_y_pst_from(
        pm::AbstractDCPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tm)
    p_fr = var(pm, n, :p, f_idx)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
function constraint_ohms_y_pst_to(
        pm::AbstractDCPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tm)
...
function constraint_switch_voltage_on_off(
        pm::AbstractDCPModel, n::Int, i, f_bus, t_bus, vad_min, vad_max)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
function constraint_ohms_yt_from_on_off(
        pm::AbstractDCPModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_fr, b_fr, tr, ti, tm, vad_min, vad_max)
    p_fr = var(pm, n, :p, f_idx)
    va_fr = var(pm, n, :va, f_bus)
    va_to = var(pm, n, :va, t_bus)
...
function constraint_voltage_angle_difference_on_off(
        pm::AbstractDCPModel, n::Int, f_idx, angmin, angmax, vad_min, vad_max)
    i, f_bus, t_bus = f_idx
...
function expression_bus_voltage(pm::AbstractPowerModel, n::Int, i, am::AdmittanceMatrix)
    ...
    var(pm, n, :va)[i] = JuMP.@expression(pm.model,
        sum(f*inj_p[j] for (j, f) in inj_factors))
end
...
"""
"""
function variable_branch_power_real(pm::AbstractAPLossLessModels; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    p_expr = Dict{Any, Any}(((l, i, j), p[(l, i, j)])
    for (l, i, j) in ref(pm, nw, :arcs_from))
    p_expr = merge(p_expr, Dict(((l, j, i), -1.0*p[(l, i, j)])
    for (l, i, j) in ref(pm, nw, :arcs_from)))
    var(pm, nw)[:p] = p_expr

    report && sol_component_value_edge(pm, nw, :branch, :pf, :pt, ref(pm, nw, :arcs_from),
        ref(pm, nw, :arcs_to), p_expr)
end

"""
"""
function variable_ne_branch_power_real(
        pm::AbstractAPLossLessModels; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    # this explicit type erasure is necessary
    p_ne_expr = Dict{Any, Any}([((l, i, j), 1.0*var(pm, nw, :p_ne, (l, i, j)))
                                for (l, i, j) in ref(pm, nw, :ne_arcs_from)])
    p_ne_expr = merge(p_ne_expr,
        Dict(((l, j, i), -1.0*var(pm, nw, :p_ne, (l, i, j)))
        for (l, i, j) in ref(pm, nw, :ne_arcs_from)))
    var(pm, nw)[:p_ne] = p_ne_expr

    report &&
        sol_component_value_edge(pm, nw, :ne_branch, :pf, :pt, ref(pm, nw, :ne_arcs_from),
            ref(pm, nw, :ne_arcs_to), p_ne_expr)
end

"""
"""
function constraint_network_power_balance(
        pm::AbstractAPLossLessModels, n::Int, i, comp_gen_ids, comp_pd,
        comp_qd, comp_gs, comp_bs, comp_branch_g, comp_branch_b)
    pg = var(pm, n, :pg)
...
function constraint_ohms_yt_to_on_off(
        pm::AbstractAPLossLessModels, n::Int, i, f_bus, t_bus, f_idx,
...
function constraint_thermal_limit_to_on_off(
        pm::AbstractAPLossLessModels, n::Int, i, t_idx, rate_a)
end
...
nothing to do, this model is symetric
"""
function constraint_ohms_yt_to(pm::AbstractAPLossLessModels, n::Int, f_bus,
        t_bus, f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm)
end
...
"""
"""
function constraint_storage_on_off(pm::AbstractAPLossLessModels, n::Int, i, pmin,
        pmax, qmin, qmax, charge_ub, discharge_ub)
...
"""
"""
function constraint_storage_losses(pm::AbstractAPLossLessModels, n::Int, i,
        bus, r, x, p_loss, q_loss; conductors = [1])
    ps = var(pm, n, :ps, i)
    sc = var(pm, n, :sc, i)
    sd = var(pm, n, :sd, i)
...
function constraint_voltage_angle_difference(
        pm::AbstractNFAModel, n::Int, f_idx, angmin, angmax)
end

"""
nothing to do, no voltage angle variables
"""
function constraint_ohms_yt_from(pm::AbstractNFAModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
end
...
Creates Ohms constraints (yt post fix indicates that Y and T values are in rectangular form)
"""
function constraint_ohms_yt_to(pm::AbstractDCPLLModel, n::Int, f_bus, t_bus,
...
"""
function constraint_ohms_yt_to_on_off(
        pm::AbstractDCPLLModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_to, b_to, tr, ti, tm, vad_min, vad_max)
    p_fr = var(pm, n, :p, f_idx)
    p_to = var(pm, n, :p, t_idx)
    va_fr = var(pm, n, :va, f_bus)
...
"""
"""
function constraint_ne_ohms_yt_to(pm::AbstractDCPLLModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_to, b_to, tr, ti, tm, vad_min, vad_max)
    p_fr = var(pm, n, :p_ne, f_idx)
