...
"""
"""
function variable_branch_current(pm::AbstractIVRModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true, kwargs...)
    variable_branch_current_real(pm, nw = nw, bounded = bounded, report = report; kwargs...)
    variable_branch_current_imaginary(
        pm, nw = nw, bounded = bounded, report = report; kwargs...)
    ...
    variable_branch_series_current_real(
        pm, nw = nw, bounded = bounded, report = report; kwargs...)
    variable_branch_series_current_imaginary(
        pm, nw = nw, bounded = bounded, report = report; kwargs...)
end

"""
"""
function variable_gen_current(pm::AbstractIVRModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true, kwargs...)
    variable_gen_current_real(pm, nw = nw, bounded = bounded, report = report; kwargs...)
    variable_gen_current_imaginary(
        pm, nw = nw, bounded = bounded, report = report; kwargs...)
...
"""
"""
function variable_dcline_current(pm::AbstractIVRModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true, kwargs...)
    variable_dcline_current_real(pm, nw = nw, bounded = bounded, report = report; kwargs...)
    variable_dcline_current_imaginary(
        pm, nw = nw, bounded = bounded, report = report; kwargs...)
    # store expressions in rectangular power variable space
    p = Dict()
    q = Dict()
...
"""
function constraint_current_from(
        pm::AbstractIVRModel, n::Int, f_bus, f_idx, g_sh_fr, b_sh_fr, tr, ti, tm)
    ...
    JuMP.@constraint(pm.model,
        cr_fr == (tr*csr_fr - ti*csi_fr + g_sh_fr*vr_fr - b_sh_fr*vi_fr)/tm^2)
    JuMP.@constraint(pm.model,
        ci_fr == (tr*csi_fr + ti*csr_fr + g_sh_fr*vi_fr + b_sh_fr*vr_fr)/tm^2)
end

"""
Defines how current distributes over series and shunt impedances of a pi-model branch
"""
function constraint_current_to(
        pm::AbstractIVRModel, n::Int, t_bus, f_idx, t_idx, g_sh_to, b_sh_to)
...
"""
function constraint_voltage_drop(
        pm::AbstractIVRModel, n::Int, i, f_bus, t_bus, f_idx, r, x, tr, ti, tm)
    vr_fr = var(pm, n, :vr, f_bus)
    vi_fr = var(pm, n, :vi, f_bus)
...
function constraint_voltage_angle_difference(
        pm::AbstractIVRModel, n::Int, f_idx, angmin, angmax)
    i, f_bus, t_bus = f_idx
...
`sum(cr + im*ci) = 0`
"""
function constraint_current_balance(pm::AbstractIVRModel, n::Int, i, bus_arcs,
        bus_arcs_dc, bus_gens, bus_pd, bus_qd, bus_gs, bus_bs)
    vr = var(pm, n, :vr, i)
    vi = var(pm, n, :vi, i)
    ...
    JuMP.@NLconstraint(pm.model,
        ...
        (sum(pd for pd in values(bus_pd))*vr +
         sum(qd for qd in values(bus_qd))*vi)/(vr^2 + vi^2)
        -
    ...
    JuMP.@NLconstraint(pm.model,
        ...
        (sum(pd for pd in values(bus_pd))*vi -
         sum(qd for qd in values(bus_qd))*vr)/(vr^2 + vi^2)
        -
...
function constraint_dcline_power_losses(
        pm::AbstractIVRModel, n::Int, f_bus, t_bus, f_idx, t_idx, loss0, loss1)
    vr_fr = var(pm, n, :vr, f_bus)
    vi_fr = var(pm, n, :vi, f_bus)
...
"""
function constraint_dcline_power_fr_bounds(
        pm::AbstractIVRModel, n::Int, i, f_bus, f_idx, pmax, pmin, qmax, qmin)
    vr_fr = var(pm, n, :vr, f_bus)
    vi_fr = var(pm, n, :vi, f_bus)
...
"""
function constraint_dcline_power_to_bounds(
        pm::AbstractIVRModel, n::Int, i, t_bus, t_idx, pmax, pmin, qmax, qmin)
    vr_to = var(pm, n, :vr, t_bus)
    vi_to = var(pm, n, :vi, t_bus)
...
"""
function constraint_dcline_setpoint_active(
        pm::AbstractIVRModel, n::Int, f_idx, t_idx, pref_fr, pref_to)
    (l, f_bus, t_bus) = f_idx
    vr_fr = var(pm, n, :vr, f_bus)
    vi_fr = var(pm, n, :vi, f_bus)
    ...
    for (n, nw_ref) in nws(pm)
        for (i, gen) in nw_ref[:gen]
            ...
            elseif length(gen["cost"]) == 2
                gen_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    gen["cost"][1]*sum(pg[c] for c in 1:nc) + gen["cost"][2])
            elseif length(gen["cost"]) == 3
                gen_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    gen["cost"][1]*sum(pg[c] for c in 1:nc)^2 +
                    gen["cost"][2]*sum(pg[c] for c in 1:nc) + gen["cost"][3])
            else
                gen_cost[(n, i)] = 0.0
            end
        end
        ...
        for (i, dcline) in nw_ref[:dcline]
            ...
            elseif length(dcline["cost"]) == 2
                dcline_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    dcline["cost"][1]*sum(p_dc[c] for c in 1:nc) + dcline["cost"][2])
            elseif length(dcline["cost"]) == 3
                dcline_cost[(n, i)] = JuMP.@NLexpression(pm.model,
                    dcline["cost"][1]*sum(p_dc[c] for c in 1:nc)^2 +
                    dcline["cost"][2]*sum(p_dc[c] for c in 1:nc) + dcline["cost"][3])
            else
                dcline_cost[(n, i)] = 0.0
            end
        end
    ...
    for (n, nw_ref) in nws(pm)
        ...
        for (i, gen) in nw_ref[:gen]
            ...
            for line in gen_lines[i]
                JuMP.@NLconstraint(pm.model,
                    pg_cost[i] >= line.slope*sum(pg[c] for c in 1:nc) + line.intercept)
            end
        end
    end
end
...
function objective_variable_dc_cost(pm::AbstractIVRModel; report::Bool = true)
    for (n, nw_ref) in nws(pm)
        ...
        report &&
            sol_component_value(pm, n, :dcline, :p_dc_cost, ids(pm, n, :dcline), dc_p_cost)
        ...
        for (i, dcline) in nw_ref[:dcline]
            ...
            for line in dcline_lines[i]
                ...
                JuMP.@NLconstraint(pm.model,
                    dc_p_cost[i] >= line.slope*sum(p_dc[c] for c in 1:nc) + line.intercept)
            end
        end
    end
end
