...
function variable_buspair_current_magnitude_sqr(
        pm::AbstractBFModel; nw::Int = nw_id_default,
...
"""
function constraint_power_losses(pm::AbstractBFModel, n::Int, i, f_bus, t_bus, f_idx,
    ...
    JuMP.@constraint(pm.model,
        p_fr + p_to ==
        r*(ccm + ym_sh_sqr*(w_fr/tm^2) - 2*(g_sh_fr*p_fr - b_sh_fr*q_fr)) +
        g_sh_fr*(w_fr/tm^2) + g_sh_to*w_to)
    JuMP.@constraint(pm.model,
        q_fr + q_to ==
        x*(ccm + ym_sh_sqr*(w_fr/tm^2) - 2*(g_sh_fr*p_fr - b_sh_fr*q_fr)) -
        b_sh_fr*(w_fr/tm^2) - b_sh_to*w_to)
end

"""
Defines voltage drop over a branch, linking from and to side voltage magnitude
"""
function constraint_voltage_magnitude_difference(
        pm::AbstractBFModel, n::Int, i, f_bus, t_bus,
        f_idx, t_idx, r, x, g_sh_fr, b_sh_fr, tm)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
    w_fr = var(pm, n, :w, f_bus)
    w_to = var(pm, n, :w, t_bus)
    ...
    JuMP.@constraint(pm.model,
        (1+2*(r*g_sh_fr - x*b_sh_fr))*(w_fr/tm^2) - w_to ==
        2*(r*p_fr + x*q_fr) -
        (r^2 + x^2)*(ccm + ym_sh_sqr*(w_fr/tm^2) - 2*(g_sh_fr*p_fr - b_sh_fr*q_fr)))
end
...
function constraint_model_current(pm::AbstractBFConicModel, n::Int)
    ...
    for (i, branch) in ref(pm, n, :branch)
        ...
        JuMP.@constraint(pm.model,
            [w[f_bus]/tm^2, ccm[i]/2, p[f_idx], q[f_idx]] in JuMP.RotatedSecondOrderCone())
    end
end

function constraint_voltage_angle_difference(
        pm::AbstractBFModel, n::Int, f_idx, angmin, angmax)
    i, f_bus, t_bus = f_idx
    t_idx = (i, t_bus, f_bus)
...
"""
function constraint_power_losses(pm::AbstractBFAModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, r, x, g_sh_fr, g_sh_to, b_sh_fr, b_sh_to, tm)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
    p_to = var(pm, n, :p, t_idx)
...
"""
function constraint_voltage_magnitude_difference(
        pm::AbstractBFAModel, n::Int, i, f_bus, t_bus,
        f_idx, t_idx, r, x, g_sh_fr, b_sh_fr, tm)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
    w_fr = var(pm, n, :w, f_bus)
...
"""
function variable_buspair_current_magnitude_sqr(
        pm::AbstractBFAModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
end
...
function constraint_storage_losses(
        pm::AbstractBFAModel, n::Int, i, bus, r, x, p_loss, q_loss; conductors = [1])
    ps = var(pm, n, :ps, i)
    qs = var(pm, n, :qs, i)
    sc = var(pm, n, :sc, i)
    sd = var(pm, n, :sd, i)
