...
```
"""
function constraint_ne_ohms_yt_from(pm::AbstractWRModel, n::Int, i, f_bus, t_bus, f_idx,
    ...
    JuMP.@constraint(pm.model,
        p_fr == (g+g_fr)/tm^2*w_fr + (-g*tr+b*ti)/tm^2*wr + (-b*tr-g*ti)/tm^2*wi)
    JuMP.@constraint(pm.model,
        q_fr == -(b+b_fr)/tm^2*w_fr - (-b*tr-g*ti)/tm^2*wr + (-g*tr+b*ti)/tm^2*wi)
end
...
```
"""
function constraint_ne_ohms_yt_to(pm::AbstractWRModel, n::Int, i, f_bus, t_bus, f_idx,
    ...
    JuMP.@constraint(pm.model,
        p_to == (g+g_to)*w_to + (-g*tr-b*ti)/tm^2*wr + (-b*tr+g*ti)/tm^2*-wi)
    JuMP.@constraint(pm.model,
        q_to == -(b+b_to)*w_to - (-b*tr+g*ti)/tm^2*wr + (-g*tr-b*ti)/tm^2*-wi)
end
...
function constraint_ne_model_voltage(pm::AbstractWRModel, n::Int)
    ...
    wi_max = ref_calc_voltage_product_bounds(ref(pm, n, :ne_buspairs))
...
function constraint_ohms_yt_from_on_off(
        pm::AbstractWRModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_fr, b_fr, tr, ti, tm, vad_min, vad_max)
...
```
"""
function constraint_ohms_yt_to_on_off(pm::AbstractWRModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_to, b_to, tr, ti, tm, vad_min, vad_max)
...
function constraint_voltage_angle_difference_on_off(
        pm::AbstractWRModel, n::Int, f_idx, angmin, angmax, vad_min, vad_max)
    i, f_bus, t_bus = f_idx
    wr = var(pm, n, :wr, i)
    wi = var(pm, n, :wi, i)
...
"""
"""
function variable_ne_branch_voltage_magnitude_fr_sqr(
        pm::AbstractWRModel; nw::Int = nw_id_default, report::Bool = true)
    buses = ref(pm, nw, :bus)
    ...
    report &&
        sol_component_value(pm, nw, :ne_branch, :w_fr, ids(pm, nw, :ne_branch), w_fr_ne)
end

"""
"""
function variable_ne_branch_voltage_magnitude_to_sqr(
        pm::AbstractWRModel; nw::Int = nw_id_default, report::Bool = true)
    buses = ref(pm, nw, :bus)
    branches = ref(pm, nw, :ne_branch)
    ...
    report &&
        sol_component_value(pm, nw, :ne_branch, :w_to, ids(pm, nw, :ne_branch), w_to_ne)
end
...
function variable_ne_branch_voltage_product(pm::AbstractWRModel; nw::Int = nw_id_default, report::Bool = true)
    wr_min, wr_max, wi_min,
    wi_max = ref_calc_voltage_product_bounds(ref(pm, nw, :ne_buspairs))
...
"""
function variable_buspair_voltage_product_angle(
    ...
    report &&
        sol_component_value_buspair(pm, nw, :buspairs, :td, ids(pm, nw, :buspairs), td)
end

"""
Creates the voltage magnitude product variables
"""
function variable_buspair_voltage_product_magnitude(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report &&
        sol_component_value_buspair(pm, nw, :buspairs, :vv, ids(pm, nw, :buspairs), vv)
end

"""
"""
function variable_buspair_current_magnitude_sqr(
        pm::AbstractPowerModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    ...
    report &&
        sol_component_value_buspair(pm, nw, :buspairs, :ccm, ids(pm, nw, :buspairs), ccm)
end
...
"""
function constraint_power_magnitude_sqr(
        pm::AbstractQCWRModel, n::Int, f_bus, t_bus, arc_from, tm)
...
"""
function constraint_power_magnitude_link(pm::AbstractQCWRModel, n::Int, f_bus, t_bus,
        arc_from, g, b, g_fr, b_fr, g_to, b_to, tr, ti, tm)
    ...
    JuMP.@constraint(pm.model,
        ccm ==
        (g^2 + b^2)*(w_fr/tm^2 + w_to - 2*(tr*wr + ti*wi)/tm^2) - ym_sh_sqr*(w_fr/tm^2) +
        2*(g_fr*p_fr - b_fr*q_fr))
end
...
function constraint_voltage_angle_difference(
        pm::AbstractQCWRModel, n::Int, f_idx, angmin, angmax)
    i, f_bus, t_bus = f_idx
...
function variable_ne_branch_voltage(pm::AbstractQCWRModel; kwargs...)
    Memento.error(_LOGGER,
        "variable_ne_branch_voltage is not yet supported for QC formulations, open an issue if you would like this feature.")
end

"""
"""
function variable_branch_voltage_product_angle_on_off(
...
"""
"""
function variable_buspair_voltage_product_magnitude_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
    branches = ref(pm, nw, :branch)
    buses = ref(pm, nw, :bus)

    vv_min = Dict((l, buses[branch["f_bus"]]["vmin"]*buses[branch["t_bus"]]["vmin"])
    for (l, branch) in branches)
    vv_max = Dict((l, buses[branch["f_bus"]]["vmax"]*buses[branch["t_bus"]]["vmax"])
    for (l, branch) in branches)
...
"""
"""
function variable_branch_current_magnitude_sqr_on_off(
        pm::AbstractPowerModel; nw::Int = nw_id_default, report::Bool = true)
...
"""
function constraint_power_magnitude_sqr_on_off(
        pm::AbstractQCWRModel, n::Int, i, f_bus, arc_from, tm)
    w = var(pm, n, :w, f_bus)
    p_fr = var(pm, n, :p, arc_from)
    q_fr = var(pm, n, :q, arc_from)
    ccm = var(pm, n, :ccm, i)
...
"""
function constraint_power_magnitude_link_on_off(pm::AbstractQCWRModel, n::Int, i, arc_from,
        g, b, g_fr, b_fr, g_to, b_to, tr, ti, tm)
    w_fr = var(pm, n, :w_fr, i)
    w_to = var(pm, n, :w_to, i)
    p_fr = var(pm, n, :p, arc_from)
    q_fr = var(pm, n, :q, arc_from)
...
"""
function variable_buspair_voltage_product_magnitude(
        pm::AbstractQCLSModel; nw::Int = nw_id_default, report::Bool = true)
...
"""
function variable_buspair_voltage_product_magnitude_multipliers(
        pm::AbstractQCLSModel; nw::Int = nw_id_default, report::Bool = true)
...
function constraint_model_voltage(pm::AbstractQCLSModel, n::Int)
    _check_missing_keys(
        var(pm, n), [
            :vm, :va, :td, :si, :cs, :w, :wr, :wi, :lambda_wr, :lambda_wi],
        typeof(pm))
