...
function constraint_power_balance(
        pm::AbstractACRModel, n::Int, i::Int, bus_arcs, bus_arcs_dc,
...
"""
"""
function expression_branch_power_ohms_yt_from(pm::AbstractACRModel, n::Int, f_bus, t_bus,
    ...
    var(pm, n, :p)[f_idx] = (g+g_fr)/tm^2*(vr_fr^2 + vi_fr^2) +
                            (-g*tr+b*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) +
                            (-b*tr-g*ti)/tm^2*(vi_fr*vr_to - vr_fr*vi_to)
    var(pm, n, :q)[f_idx] = -(b+b_fr)/tm^2*(vr_fr^2 + vi_fr^2) -
...
"""
"""
function expression_branch_power_ohms_yt_to(pm::AbstractACRModel, n::Int, f_bus, t_bus,
    ...
    var(pm, n, :p)[t_idx] = (g+g_to)*(vr_to^2 + vi_to^2) +
                            (-g*tr-b*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) +
                            (-b*tr+g*ti)/tm^2*(-(vi_fr*vr_to - vr_fr*vi_to))
    var(pm, n, :q)[t_idx] = -(b+b_to)*(vr_to^2 + vi_to^2) -
                            (-b*tr+g*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) +
                            (-g*tr-b*ti)/tm^2*(-(vi_fr*vr_to - vr_fr*vi_to))
end
...
"""
function constraint_ohms_yt_from(pm::AbstractACRModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
    ...
    JuMP.@constraint(pm.model,
        p_fr ==
        (g+g_fr)/tm^2*(vr_fr^2 + vi_fr^2) + (-g*tr+b*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) +
        (-b*tr-g*ti)/tm^2*(vi_fr*vr_to - vr_fr*vi_to))
    JuMP.@constraint(pm.model,
        q_fr ==
        -(b+b_fr)/tm^2*(vr_fr^2 + vi_fr^2) - (-b*tr-g*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) +
        (-g*tr+b*ti)/tm^2*(vi_fr*vr_to - vr_fr*vi_to))
end
...
Creates Ohms constraints (yt post fix indicates that Y and T values are in rectangular form)
"""
function constraint_ohms_yt_to(pm::AbstractACRModel, n::Int, f_bus, t_bus,
        f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm)
    ...
    JuMP.@constraint(pm.model,
        p_to ==
        (g+g_to)*(vr_to^2 + vi_to^2) + (-g*tr-b*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) +
        (-b*tr+g*ti)/tm^2*(-(vi_fr*vr_to - vr_fr*vi_to)))
    JuMP.@constraint(pm.model,
        q_to ==
        -(b+b_to)*(vr_to^2 + vi_to^2) - (-b*tr+g*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) +
        (-g*tr-b*ti)/tm^2*(-(vi_fr*vr_to - vr_fr*vi_to)))
end
...
function constraint_storage_losses(
        pm::AbstractACRModel, n::Int, i, bus, r, x, p_loss, q_loss; conductors = [1])
    vr = var(pm, n, :vr, bus)
    vi = var(pm, n, :vi, bus)
    ps = var(pm, n, :ps, i)
    qs = var(pm, n, :qs, i)
    ...
    JuMP.@NLconstraint(pm.model,
        ...
        ==
        qsc + q_loss +
        sum(x[c]*(ps[c]^2 + qs[c]^2)/(vr[c]^2 + vi[c]^2) for c in conductors))
end
...
function constraint_voltage_angle_difference(
        pm::AbstractACRModel, n::Int, f_idx, angmin, angmax)
    i, f_bus, t_bus = f_idx
    ...
    JuMP.@constraint(pm.model,
        (vi_fr*vr_to - vr_fr*vi_to) <= tan(angmax)*(vr_fr*vr_to + vi_fr*vi_to))
    JuMP.@constraint(pm.model,
        (vi_fr*vr_to - vr_fr*vi_to) >= tan(angmin)*(vr_fr*vr_to + vi_fr*vi_to))
end
