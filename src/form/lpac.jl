...
"""
"""
function variable_bus_voltage_magnitude(pm::AbstractLPACModel; nw::Int = nw_id_default,
...
function constraint_power_balance(
        pm::AbstractLPACModel, n::Int, i::Int, bus_arcs, bus_arcs_dc,
...
"""
"""
function constraint_ohms_yt_from(pm::AbstractLPACCModel, n::Int, f_bus, t_bus,
    ...
    JuMP.@constraint(pm.model,
        p_fr ==
        (g+g_fr)/tm^2*(1.0 + 2*phi_fr) + (-g*tr+b*ti)/tm^2*(cs + phi_fr + phi_to) +
        (-b*tr-g*ti)/tm^2*(va_fr-va_to))
    JuMP.@constraint(pm.model,
        q_fr ==
        -(b+b_fr)/tm^2*(1.0 + 2*phi_fr) - (-b*tr-g*ti)/tm^2*(cs + phi_fr + phi_to) +
        (-g*tr+b*ti)/tm^2*(va_fr-va_to))
end

"""
"""
function constraint_ohms_yt_to(pm::AbstractLPACCModel, n::Int, f_bus, t_bus,
    ...
    JuMP.@constraint(pm.model,
        p_to ==
        (g+g_to)*(1.0 + 2*phi_to) + (-g*tr-b*ti)/tm^2*(cs + phi_fr + phi_to) +
        (-b*tr+g*ti)/tm^2*-(va_fr-va_to))
    JuMP.@constraint(pm.model,
        q_to ==
        -(b+b_to)*(1.0 + 2*phi_to) - (-b*tr+g*ti)/tm^2*(cs + phi_fr + phi_to) +
        (-g*tr-b*ti)/tm^2*-(va_fr-va_to))
end
...
function constraint_ne_voltage_angle_difference(
        pm::AbstractLPACCModel, n::Int, f_idx, angmin, angmax, vad_min, vad_max)
    i, f_bus, t_bus = f_idx
...
function variable_ne_branch_voltage_magnitude_fr(
        pm::AbstractLPACModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
    buses = ref(pm, nw, :bus)
...
function variable_ne_branch_voltage_product_angle(
        pm::AbstractLPACCModel; nw::Int = nw_id_default,
        bounded::Bool = true, report::Bool = true)
...
function constraint_ne_power_balance(
        pm::AbstractLPACCModel, n::Int, i::Int, bus_arcs, bus_arcs_dc, bus_arcs_sw,
...
"""
"""
function constraint_ne_ohms_yt_from(pm::AbstractLPACCModel, n::Int, i, f_bus, t_bus, f_idx,
    ...
    JuMP.@constraint(pm.model,
        p_fr ==
        (g+g_fr)/tm^2*(z + 2*phi_fr) + (-g*tr+b*ti)/tm^2*(cs + phi_fr + phi_to) +
        (-b*tr-g*ti)/tm^2*(td))
    JuMP.@constraint(pm.model,
        q_fr ==
        -(b+b_fr)/tm^2*(z + 2*phi_fr) - (-b*tr-g*ti)/tm^2*(cs + phi_fr + phi_to) +
        (-g*tr+b*ti)/tm^2*(td))
end

"""
"""
function constraint_ne_ohms_yt_to(pm::AbstractLPACCModel, n::Int, i, f_bus, t_bus, f_idx,
    ...
    JuMP.@constraint(pm.model,
        q_to ==
        -(b+b_to)*(z + 2*phi_to) - (-b*tr+g*ti)/tm^2*(cs + phi_fr + phi_to) +
        (-g*tr-b*ti)/tm^2*-(td))
end
...
function variable_branch_voltage_magnitude_fr_on_off(
        pm::AbstractLPACCModel; nw::Int = nw_id_default, report::Bool = true)
    buses = ref(pm, nw, :bus)
...
function constraint_ohms_yt_from_on_off(
        pm::AbstractLPACCModel, n::Int, i, f_bus, t_bus, f_idx,
        t_idx, g, b, g_fr, b_fr, tr, ti, tm, vad_min, vad_max)
    p_fr = var(pm, n, :p, f_idx)
    q_fr = var(pm, n, :q, f_idx)
