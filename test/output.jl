...
@testset "test solution processors" begin
    @testset "sol_vr_to_vp" begin
        result = run_opf("../test/data/matpower/case5.m", ACRPowerModel,
    ...
    @testset "sol_phi_to_vm" begin
        result = run_opf("../test/data/matpower/case5.m", LPACCPowerModel,
            nlp_solver, solution_processors = [sol_data_model!])
...
@testset "test solution feedback" begin
    function solution_feedback(case, ac_opf_obj)
        ...
        for (i, bus) in data["bus"]
            @test isapprox(opf_result["solution"]["bus"][i]["va"],
                pf_result["solution"]["bus"][i]["va"]; atol = 1e-3)
            @test isapprox(opf_result["solution"]["bus"][i]["vm"],
                pf_result["solution"]["bus"][i]["vm"]; atol = 1e-3)
        end

        for (i, gen) in data["gen"]
            @test isapprox(opf_result["solution"]["gen"][i]["pg"],
                pf_result["solution"]["gen"][i]["pg"]; atol = 1e-3)
        ...
        for (i, dcline) in data["dcline"]
            @test isapprox(opf_result["solution"]["dcline"][i]["pf"],
                pf_result["solution"]["dcline"][i]["pf"]; atol = 1e-3)
            @test isapprox(opf_result["solution"]["dcline"][i]["pt"],
                pf_result["solution"]["dcline"][i]["pt"]; atol = 1e-3)
        end
    end
