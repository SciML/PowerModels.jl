...
@testset "obbt with trilinear convex hull relaxation" begin
    @testset "3-bus case" begin
        ...
        data,
        stats = run_obbt_opf!("../test/data/matpower/case3.m", nlp_solver, model_type = QCLSPowerModel);
        ...
        data,
        stats = run_obbt_opf!("../test/data/matpower/case3.m", nlp_solver,
...
@testset "obbt with qc relaxation" begin
    @testset "3-bus case" begin
        ...
        data,
        stats = run_obbt_opf!("../test/data/matpower/case3.m", nlp_solver, model_type = QCRMPowerModel);
    ...
    @testset "3-bus linear case" begin
        ...
        data,
        stats = run_obbt_opf!(data, nlp_solver,
            model_type = QCRMPowerModel,
            upper_bound = upper_bound,
            upper_bound_constraint = true);
...
@testset "opf with flow cuts" begin
    @testset "ac 5-bus case" begin
        ...
        for (i, bus) in result_base["solution"]["bus"]
            @test isapprox(result_base["solution"]["bus"][i]["vm"],
                result_cuts["solution"]["bus"][i]["vm"]; atol = 1e-7)
            @test isapprox(result_base["solution"]["bus"][i]["va"],
                result_cuts["solution"]["bus"][i]["va"]; atol = 1e-7)
        end
    end
    @testset "ac 14-bus case" begin
        ...
        for (i, bus) in result_base["solution"]["bus"]
            @test isapprox(result_base["solution"]["bus"][i]["vm"],
                result_cuts["solution"]["bus"][i]["vm"]; atol = 1e-8)
            @test isapprox(result_base["solution"]["bus"][i]["va"],
                result_cuts["solution"]["bus"][i]["va"]; atol = 1e-8)
        end
    end
    ...
    @testset "soc 5-bus case" begin
        ...
        @test isapprox(result_base["objective"], result_cuts["objective"])
        for (i, bus) in result_base["solution"]["bus"]
            @test isapprox(result_base["solution"]["bus"][i]["w"],
                result_cuts["solution"]["bus"][i]["w"]; atol = 1e-5)
        end
    end
...
@testset "ptdf opf with flow cuts" begin
    @testset "dc 5-bus case" begin
        ...
        for (i, gen) in result_base["solution"]["gen"]
            @test isapprox(result_base["solution"]["gen"][i]["pg"],
                result_cuts["solution"]["gen"][i]["pg"]; atol = 1e-8)
        end
    end
